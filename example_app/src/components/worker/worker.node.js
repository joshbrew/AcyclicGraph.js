...
export class WorkerNode extends NodeDiv {
    props={
        workerid:manager.addWorker(), //or set workerId in 
        callbackName:'ping',
        operator:async (
            ...
            res = await manager.run(this.callbackName,input,node.tag); //pretty simple, run the callback and pass the input/origin
            return res;   
    ...
    set callbackName(callbackName='ping') {
...
addCustomElement(TemplateNodeDiv,'thread-node');
